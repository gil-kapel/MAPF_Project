...
def calc_sum_of_costs(paths):
    sum_of_costs = 0
    for path in paths:
        sum_of_costs += len(path) - 1
    return sum_of_costs


def calc_make_span(paths):
    max_time = -np.inf
    for path in paths:
...
def detect_collisions(paths):
    ...
    for i, path1 in enumerate(paths):
        for j, path2 in enumerate(paths):
            if path1 == path2:
                continue
            collision = detect_collision(i, path1, j, path2)
...
def detect_collision(agent1: int, path1: Path, agent2: int, path2: Path):
    for t, loc in enumerate(path1):
        if loc == get_location(path2, t):
            collision = Collision(agent1, agent2, t, loc)
            return collision
        elif loc == get_location(path2, t + 1) and \
                get_location(path1, t + 1) == get_location(path2, t):
            collision = Collision(agent1, agent2, t + 1, loc, get_location(path1, t + 1))
            return collision
    return None
...
def is_collision_in_list(collision: Collision, collisions) -> bool:
...
class CBSInput(MAPFInput):
    ...
    def validate_input(self):
        if self.num_of_agents < 0 or not self.are_agents_in_legal_places():
            raise WrongInput('Illegal input')
...
class DictCBSSolver(MAPFSolver):
    """CBS high-level search implemented with a dictionary nodes"""
    ...
    def solve(self, cbs_input: CBSInput) -> MAPFOutput:
        ...
        root = {'sum_of_costs': 0, 'make_span': 0, 'constraints': [], 'paths': [], 'collisions': []}
        ...
        root["sum_of_costs"] = calc_sum_of_costs(root['paths'])
        root["make_span"] = calc_make_span(root['paths'])
        root['collisions'] = detect_collisions(root['paths'])
        self.push_node(root)
        while len(self.open_list) > 0:
            smallest_node = self.__pop_node()
            if timer.time() - self.start_time > self.time_limit * 1.2:
                raise TimeLimit(MAPFOutput(smallest_node["paths"], smallest_node["sum_of_costs"],
                                  smallest_node["make_span"], self.CPU_time))
            if len(smallest_node['collisions']) == 0:
                ...
                return MAPFOutput(smallest_node["paths"], smallest_node["sum_of_costs"],
                                  smallest_node["make_span"], self.CPU_time)
            ...
            for constraint in constraints:
                ...
                new_node = {'sum_of_costs': smallest_node['sum_of_costs'],
                            'make_span': smallest_node['make_span'],
                            'constraints': new_constraints,
                            'paths': copy.deepcopy(smallest_node['paths']),
                            'collisions': []}
                agent = constraint.agent
                goal = cbs_input.goals_list[agent]
                lls_input = self.low_level_search.input_factory(cbs_input.map_instance,
                                                                cbs_input.starts_list[agent],
                                                                goal, 0, agent, new_node["constraints"],
                                                                self.heuristics[goal])
                path = self.low_level_search.search(lls_input)
                if path is not None:
                    new_node['paths'][agent] = path
                    new_node['collisions'] = detect_collisions(new_node['paths'])
                    new_node['sum_of_costs'] = calc_sum_of_costs(new_node['paths'])
                    new_node['make_span'] = calc_make_span(new_node['paths'])
    ...
    def push_node(self, node):
        heapq.heappush(self.open_list, (node["sum_of_costs"], node["make_span"], len(node["collisions"]), self.num_of_generated, node))
